...
if (isset($_POST['asset_submit']) && !empty($_POST['asset_submit'])) {
	...
	$mrrcode	= 	$short_name.'-';
	$slCode 	= 	getDefaultCategoryCodeByStore('ams_products', 'sl_no', '03d', '001', $mrrcode);
    /// Assets Code
...
if(isset($_POST['service_submit'])){
	...
	$department_id 	= $_POST['department_id'];
	$proloc_id 		= $_POST['proloc_id'];
	$store_id 		= $_POST['current_store'];

		/* Insert Data Into product_assign Table: */
		
		$query = "INSERT INTO `inv_services`(`srv_no`,`assets_id`,`assets_slno`,`warranty`,`vendor`,`handover_date`,`handover_by`,`status`,`ho_remarks`,`division_id`,`department_id`,`proloc_id`,`store_id`) VALUES ('$srv_no','$assets_id','$assets_slno','$warranty','$vendor','$handover_date','$handover_by','$status','$ho_remarks','$division_id','$department_id','$proloc_id','$store_id')";
        $conn->query($query);
...
if(isset($_POST['service_update_submit'])){
	...
	$update_at 	= date("Y-m-d");
		...
		$bilquery = "INSERT INTO `inv_services_bill`(`srv_ref`,`bill_no`, `bill_date`, `amount`,`assets_slno`, `vendor_id`, `division_id`, `department_id`, `proloc_id`, `store_id`,`remarks`, `created_by`) VALUES ('$srv_ref','$bill_no','$bill_date','$amount','$assets_slno','$vendor_id','$division_id','$department_id','-','$store_id','$remarks','$created_by')";
        $conn->query($bilquery);
		...
		$query = "UPDATE `inv_services` set `status`='$status',`receive_date`='$bill_date',`receive_by`='$created_by',`recv_remarks`='$remarks',`updated_at`='$update_at' where `id`='$id'";
        $conn->query($query);
