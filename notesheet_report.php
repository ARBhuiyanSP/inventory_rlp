...
<div class="container-fluid">
    ...
    <ol class="breadcrumb">
        ...
        <li class="breadcrumb-item active"> Notesheet Reports</li>
    </ol>
    <!-- DataTables Example -->
    <div class="card mb-3">
...
if(isset($_POST['submit'])){
	...
	$resultSet = mysqli_query($conn, "SELECT * FROM `notesheets_master` WHERE `is_delete` =  '0'".($project_id!='all'?" AND `request_project` = '$project_id'":'')." AND `created_at` BETWEEN '$from_date' AND '$to_date' ");
...
echo "</pre>"; */
	...
	if($resultSet->num_rows > 0){
		...
		while($rows = $resultSet->fetch_assoc()) {
			$i++;
			echo "<tr>
					<td><a href='notesheets_view.php?id=" . $rows['id'] . "' target='blank'>" . $rows['notesheet_no'] . "</a></td>
					<td>" . getProjectNameByID($rows['request_project']) . "</td>
					<td>" . date('jS F Y', strtotime($rows['created_at'])) . "</td>
					<td>" . get_status_name($rows['notesheet_status']) . "</td>
				</tr>";
