...
//Create User:
if (isset($_POST['equips_rlp_create']) && !empty($_POST['equips_rlp_create'])){
    ...
    $rlpNo		= (isset($_POST['equips_rlp_no']) && !empty($_POST['equips_rlp_no']) ? trim(mysqli_real_escape_string($conn,$_POST['equips_rlp_no'])) : "");
    $remarks		= (isset($_POST['remarks']) && !empty($_POST['remarks']) ? trim(mysqli_real_escape_string($conn,$_POST['remarks'])) : "");
    
    /*
     * *****************************equips_rlp_info table operation********************
     */    
    $equips_rlp_info_response  =   execute_equips_equips_rlp_info_table();
    if(isset($equips_rlp_info_response) && $equips_rlp_info_response['status'] == "success"){
        $equips_rlp_info_id    =   $equips_rlp_info_response['last_id'];
        /*
        * *****************************equips_rlp_details table operation********************
        */    
        $equips_rlp_details_response  =   execute_equips_equips_rlp_details_table($equips_rlp_info_id);
        
        $ackParam['acknowledge_user']   =   $_POST['assign_users_order'];
        $ackParam['equips_rlp_info_id']        =   $equips_rlp_info_id;
        $ackParam['created_by']         =   $_SESSION['logged']['user_id'];
        process_equips_equips_rlp_acknowledgement($ackParam);
        
        $_SESSION['success']    =   "Your request have been successfully procced.";
    }else{
        $_SESSION['error']    =   "Failed to save equipments rlp data";
    }
    header("location: equips_rlp_create.php");
    exit();
}

function execute_equips_equips_rlp_info_table(){
    global $conn;
    $date		= (isset($_POST['date']) && !empty($_POST['date']) ? trim(mysqli_real_escape_string($conn,$_POST['date'])) : date("Y-m-d"));
    $priority		= (isset($_POST['priority']) && !empty($_POST['priority']) ? trim(mysqli_real_escape_string($conn,$_POST['priority'])) : "");
     ...
     * *****************************equips_rlp_info table operation********************
     */
    $table_sql     =   "equips_rlp_info";
    $dataParam     =   [
        'id'                    =>  get_table_next_primary_id($table_sql),
        //'equips_rlp_no'                =>  get_equips_rlp_no(),
        'equips_rlp_no'                =>  $rlpNo,
    ...
    $response   =   saveData("equips_rlp_info", $dataParam);
    return $response;
}

function execute_equips_equips_rlp_details_table($equips_rlp_info_id){
    global $conn;
    /*
     * ***************************** equips_rlp_details table operation ********************
     */
    for($count 		= 0; $count<count($_POST['equipments_name']); $count++){        
        ...
        $equipments_name	= (isset($_POST['equipments_name'][$count]) && !empty($_POST['equipments_name'][$count]) ? trim(mysqli_real_escape_string($conn,$_POST['equipments_name'][$count])) : '');
        ...
        $dataParam     =   [
            'id'                =>  get_table_next_primary_id('equips_rlp_details'),
            'equips_rlp_info_id'       =>  $equips_rlp_info_id,
            'equipments_name'          =>  $equipments_name,
            'purpose'           =>  $purpose,
            'quantity'          =>  $quantity,
        ];
    
        saveData("equips_rlp_details", $dataParam);
    }
}
if(isset($_GET['process_type']) && $_GET['process_type'] == "rlp_sa_supplier_update_execute"){
    include '../connection/connect.php';
    include '../helper/utilities.php';
    update_equips_supplier_details();
    $feedback   =   [
        'status'  => "success",
...
function update_equips_supplier_details(){
    global $conn;
    /*
     * *****************************equips_rlp_details table operation********************
     */
    foreach($_POST['supplier'] as $datKey=>$datValue){
        ...
        updateData('equips_rlp_details', $dataParam, $where);
    }        
...
function getEquipsRLPListData(){
    $user_id            =   $_SESSION['logged']['user_id'];
    $role       =   get_equips_rlp_role_group_short_name();
    switch ($role){
        case 'sa':
            $table      = 'equips_rlp_info WHERE is_delete = 0';
            $order      = 'DESC';
            $column     = 'created_at';
            $dataType   = 'obj';
            ...
            break;
        case 'member':
            $table      = 'equips_rlp_info WHERE is_delete = 0 AND rlp_user_id = '.$user_id;
            $order      = 'DESC';
            $column     = 'created_at';
            $dataType   = 'obj';
            $listData   = getTableDataByTableName($table, $order, $column, $dataType);
        ...
        case 'dh':
            ...
            $listData1   = getEquipsRlpInfoAcknowledgeData($user_id);
            // get own RLp:
            $table      = 'equips_rlp_info WHERE is_delete = 0 AND rlp_user_id = '.$user_id;
            $order      = 'DESC';
            $column     = 'created_at';
            ...
            break; 
        case 'ab':
            $listData   = getEquipsRlpInfoAcknowledgeData($user_id);
            break; 
        default:
            $listData   = getEquipsRlpInfoAcknowledgeData($user_id);
            break;
...
function get_equips_rlp_role_group_short_name(){
...
function getEquipsRlpInfoAcknowledgeData($user_id, $column='created_at', $order="DESC"){    
    $sql    =   "SELECT ri.*
                     FROM equips_rlp_info as ri
                     JOIN equips_rlp_acknowledgement as rack 
                     ON ri.id = rack.equips_rlp_info_id
...
if(isset($_GET['process_type']) && $_GET['process_type'] == "rlp_delete"){
    ...
    $table      =   "equips_rlp_info WHERE id=$id";
    $equips_rlp_info   = getDataRowIdAndTable($table);
    
    $table      =   "equips_rlp_info";
    $fieldName  =   "id";
    // member user can only delete pending rlp:
    if($current_user_id == $equips_rlp_info->rlp_user_id){
        if($equips_rlp_info->rlp_status == 5){            
            ...
            updateData('equips_rlp_info', $dataParam, $where);
            $dataSaveParam  =   [
                'equips_rlp_info_id'   => $id,
                'deleted_by'    => $current_user_id,
                'deleted_at'    => date("Y-d-m H:i:s")
            ];
            saveData('equips_rlp_delete_history', $dataSaveParam);
            $status                 =   "success";
        ...
        if(equips_rlp_acknowledgement_is_pending($id, $ack_status)){            
            //deleteRecordByTableAndId($table, $fieldName, $id);
            $dataParam['is_delete']      =   1;
            $dataParam['updated_by']     =   $current_user_id;
            $dataParam['updated_at']     =   date("Y-d-m H:i:s");
...
function getEquipsRlpDetailsData($rlp_id){
    $table      =   "equips_rlp_info WHERE id=$rlp_id";
    $equips_rlp_info   = getDataRowIdAndTable($table);
    
    $order = 'asc';
    $column='id';
    $table         =   "equips_rlp_details WHERE equips_rlp_info_id=$rlp_id";
    $equips_rlp_details   = getTableDataByTableNameRlp($table, $order, $column);
    
    $feedbackData   =   [
        'equips_rlp_info'      =>  $equips_rlp_info,
        'equips_rlp_details'   =>  $equips_rlp_details
    ];
    return $feedbackData;
}

function getEquipsRlpDetailsDataForNs($rlp_id){
    $table      =   "equips_rlp_info WHERE id=$rlp_id";
    $equips_rlp_info   = getDataRowIdAndTable($table);
    
    $order = 'asc';
    $column='id';
    $table         =   "equips_rlp_details WHERE equips_rlp_info_id=$rlp_id AND `is_ns`='0'";
    $equips_rlp_details   = getTableDataByTableNameRlp($table, $order, $column);
...
if(isset($_GET['process_type']) && $_GET['process_type'] == "rlp_quick_view"){
    ...
    $column='id';
    $table         =   "equips_rlp_details WHERE equips_rlp_info_id=$rlp_id";
    $equips_rlp_details   = getTableDataByTableName($table, $order, $column);
...
if(isset($_GET['process_type']) && $_GET['process_type'] == "rlp_dh_update_execute"){
    ...
    include '../connection/connect.php';
    include '../helper/utilities.php';
    if(is_equips_dh_update_field_validation_pass()){
        $param['equips_rlp_info_id']   =   $_POST['equips_rlp_info_id'];
        $param['ack_status']    =   $_POST['acknowledgement'];
        $param['user_id']       =   $_POST['created_by'];
        $param['remarks']       =   $_POST['remarks'];
        update_equips_equips_rlp_acknowledgement($param);
        save_equips_rlp_remarks();
        $feedback   =   [
            'status'    => "success",
...
if(isset($_GET['process_type']) && $_GET['process_type'] == "rlp_ab_common_update_execute"){
    ...
    include '../connection/connect.php';
    include '../helper/utilities.php';
    $param['equips_rlp_info_id']   =   $_GET['approve_id'];
    $param['ack_status']    =   '1';
    $param['user_id']       =   $_GET['user_id'];
    $param['remarks']       =   'Approved';
    update_common_equips_equips_rlp_acknowledgement($param);
    /* $dataParam['rlp_status']     =   $_POST['acknowledgement'];
    $dataParam['updated_by']     =   $_POST['created_by'];
    $dataParam['updated_at']     =   date("Y-d-m H:i:s");
    $where      =   [
        'id'    =>  $param['equips_rlp_info_id']
    ];
    updateData('equips_rlp_info', $dataParam, $where); */
    save_common_equips_rlp_remarks();
    $feedback   =   [
        'status'    => "success",
        'message'   => "RLP data have been successfully updated",
    ];
...
function update_common_equips_equips_rlp_acknowledgement($data){
   
	$user_id        =   $data['user_id'];
    $equips_rlp_info_id    =   $data['equips_rlp_info_id'];
    $dataParam['equips_rlp_info_id']       =   $data['equips_rlp_info_id'];
    $dataParam['user_id']           =   $data['user_id'];
    if(isset($data['ack_status']) && !empty($data['ack_status'])){
        $dataParam['ack_status']    =   $data['ack_status'];
    }
    $where  =   "equips_rlp_info_id=$equips_rlp_info_id AND user_id=$user_id";
    $isDuplicate    =   isDuplicateData('equips_rlp_acknowledgement', $where);
    if($isDuplicate){
        // process equips_rlp_acknowledgement (table will be updated):
        $dataParam['ack_updated_date']  =   date("Y-m-d H:i:s");
        $dataParam['updated_by']        =   $user_id;
        $where      =   [
            'id'    =>  $isDuplicate
        ];
        updateData('equips_rlp_acknowledgement', $dataParam, $where);
        ...
        $where      =   [
            'id'    =>  $equips_rlp_info_id
        ];
        updateData('equips_rlp_info', $dataParam, $where);
        
        set_rlp_visible_for_acknowledge($equips_rlp_info_id);
    }
}

function save_common_equips_rlp_remarks(){
        $dataParam      =   [];
        $dataParam['remarks']        =   'Approved';
        $dataParam['equips_rlp_info_id']    =   $_GET['approve_id'];
        $dataParam['user_id']        =   $_GET['user_id'];
        $dataParam['remarks_date']   =   date("Y-m-d H:i:s");

        saveData('equips_rlp_remarks_history', $dataParam);
    
}

// common rlp approve

function is_equips_dh_update_field_validation_pass(){
    $status     =   true;
...
if(isset($_GET['process_type']) && $_GET['process_type'] == "rlp_sa_update_execute"){
    ...
    if(isset($_POST['acknowledgement']) && !empty($_POST['acknowledgement'])){
        $equips_rlp_info_id   =   $_POST['equips_rlp_info_id'];
        if(isset($_POST['department_heads']) && !empty($_POST['department_heads'])){
    //        process equips_rlp_acknowledgement
            $param  =   [];
            $param['acknowledge_user']    =   $_POST['department_heads'];
            process_equips_equips_rlp_acknowledgement($param);
        }
    ...
    //        process equips_rlp_acknowledgement
        ...
        // process equips_rlp_info (table will be updated):
        if(isset($_POST['acknowledgement']) && !empty($_POST['acknowledgement'])){
            $dataParam['rlp_status']     =   $_POST['acknowledgement'];
        ...
        // process equips_rlp_remarks_history (table will be inserted):
        save_equips_rlp_remarks();
        $feedback   =   [
            'status'    => "success",
            'message'   => "RLP data have been successfully updated",
...
function update_equips_equips_rlp_acknowledgement($data){
    $user_id        =   $data['user_id'];
    $equips_rlp_info_id    =   $data['equips_rlp_info_id'];
    $dataParam['equips_rlp_info_id']       =   $data['equips_rlp_info_id'];
    $dataParam['user_id']           =   $data['user_id'];
...
function process_equips_equips_rlp_acknowledgement($data){
    //equips_rlp_acknowledgement will be inserted or updated!
    $equips_rlp_info_id        =   $data['equips_rlp_info_id'];
    ...
    foreach($data['acknowledge_user'] as $dtKey=>$dtValue){
        ...
        $dataParam['equips_rlp_info_id']       =   $equips_rlp_info_id;
        ...
        if($isDuplicate){
            ...
            $response   =   saveData('equips_rlp_acknowledgement', $dataParam);
        }
    }
    set_rlp_visible_for_acknowledge($equips_rlp_info_id);
}
function save_equips_rlp_remarks(){
    if(isset($_POST['remarks']) && !empty($_POST['remarks'])){
        $dataParam      =   [];
        $dataParam['remarks']        =   $_POST['remarks'];
        $dataParam['equips_rlp_info_id']    =   $_POST['equips_rlp_info_id'];
