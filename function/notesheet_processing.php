...
function execute_notesheets_master_table($file_path = "../uploads/file/"){
		...
		$type		= (isset($_POST['type']) && !empty($_POST['type']) ? trim(mysqli_real_escape_string($conn,$_POST['type'])) : "");
		
		$request_date		= (isset($_POST['request_date']) && !empty($_POST['request_date']) ? trim(mysqli_real_escape_string($conn,$_POST['request_date'])) : date("Y-m-d"));
		...
		$others		= (isset($_POST['others']) && !empty($_POST['others']) ? trim(mysqli_real_escape_string($conn,$_POST['others'])) : "");
		
		$grand_total		= (isset($_POST['grand_total']) && !empty($_POST['grand_total']) ? trim(mysqli_real_escape_string($conn,$_POST['grand_total'])) : "");
		
		$currency		= (isset($_POST['currency']) && !empty($_POST['currency']) ? trim(mysqli_real_escape_string($conn,$_POST['currency'])) : "");
