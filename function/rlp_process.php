<?php
// Rlp Query Insert
if (isset($_POST['rlp_update_while_approve']) && !empty($_POST['rlp_update_while_approve'])){
    $rlp_info_id  =   (isset($_POST['rlp_info_id']) && !empty($_POST['rlp_info_id']) ? trim(mysqli_real_escape_string($conn,$_POST['rlp_info_id'])) : "");
    $user_id  =   (isset($_POST['user_id']) && !empty($_POST['user_id']) ? trim(mysqli_real_escape_string($conn,$_POST['user_id'])) : "");
    $remarks  =   (isset($_POST['remarks']) && !empty($_POST['remarks']) ? trim(mysqli_real_escape_string($conn,$_POST['remarks'])) : "");   
	
	$rlp_info_response  =   update_rlp_info_table_while_approve($rlp_info_id);
    
	
	// first delete all from rlp_details table; 
		$delsql    = "DELETE FROM rlp_details WHERE rlp_info_id ='$rlp_info_id'";
		$conn->query($delsql);
        ...
        */    
		...
		saveData("rlp_queries", $dataParams);
	
	
        
        $_SESSION['success']    =   "Your request have been successfully procced.";
    
    header("location: rlp_update.php?rlp_id=$rlp_info_id");
    exit();
    
}
/* 
update rlp_info table
 */
 function update_rlp_info_table_while_approve($rlp_info_id){
    global $conn;
	...
	$dataParam['id']     =   $rlp_info_id;
	//$dataParam['user_remarks']     =   $remarks;
	$dataParam['totalamount']     =   $sub_total_amount;
...
// Rlp Query Insert
if (isset($_POST['query_submit']) && !empty($_POST['query_submit'])){
    $rlp_info_id  =   (isset($_POST['rlp_info_id']) && !empty($_POST['rlp_info_id']) ? trim(mysqli_real_escape_string($conn,$_POST['rlp_info_id'])) : "");
    $user_id  =   (isset($_POST['user_id']) && !empty($_POST['user_id']) ? trim(mysqli_real_escape_string($conn,$_POST['user_id'])) : "");
    $queries  =   (isset($_POST['queries']) && !empty($_POST['queries']) ? trim(mysqli_real_escape_string($conn,$_POST['queries'])) : "");   
...
//Create Convince RLP:
if (isset($_POST['conv_rlp_create']) && !empty($_POST['conv_rlp_create'])){
    ...
    header("location: conv_rlp_create.php");
    exit();
}
//Update RLP:
if (isset($_POST['rlp_update']) && !empty($_POST['rlp_update'])){
    $date		= (isset($_POST['date']) && !empty($_POST['date']) ? trim(mysqli_real_escape_string($conn,$_POST['date'])) : date("Y-m-d"));
    ...
    $rlpID		= (isset($_POST['id']) && !empty($_POST['id']) ? trim(mysqli_real_escape_string($conn,$_POST['id'])) : "");
     ...
     * *****************************rlp_info table operation ********************
     */    
    $rlp_info_response  =   update_rlp_info_table($rlpID);
    
	
	// first delete all from rlp_details table; 
		$delsql    = "DELETE FROM rlp_details WHERE rlp_info_id ='$rlpID'";
		$conn->query($delsql);
    ...
    header("location: rlp_edit.php?rlp_id=$rlp_info_id");
    exit();
}
...
update rlp_info table
 */
 function update_rlp_info_table($rlpID){
    global $conn;
    $date		= (isset($_POST['date']) && !empty($_POST['date']) ? trim(mysqli_real_escape_string($conn,$_POST['date'])) : date("Y-m-d"));
	...
	$dataParam['id']     =   $rlpID;
	$dataParam['priority']     =   $priority;
	$dataParam['user_remarks']     =   $remarks;
	$dataParam['totalamount']     =   $sub_total_amount;
	$dataParam['updated_by']     =   $_SESSION['logged']['user_id'];
	$dataParam['updated_at']     =   date('Y-m-d h:i:s');
	$dataParam['request_date']     =   date('Y-m-d h:i:s', strtotime($date));
...
function execute_rlp_details_table($rlp_info_id){
    ...
    for($count 		= 0; $count<count($_POST['material_name']); $count++){        
        ...
        $supplier	= (isset($_POST['supplier_name'][$count]) && !empty($_POST['supplier_name'][$count]) ? trim(mysqli_real_escape_string($conn,$_POST['supplier_name'][$count])) : '');        
...
// get conveyance RLP
function getConveyanceRLPListDataW(){
	...
	if($role_name == 'sa'){
			...
			$sql      = '* FROM `rlp_info` WHERE 1=1 AND `is_delete`=0 AND `rlp_type`= "Conveyance" ORDER BY `rlp_status` DESC , `created_at` DESC  ';
			...
			$sql      = 'a1.id as id,a1.rlp_no as rlp_no,a1.rlp_status as rlp_status,a2.rlp_info_id as rlp_info_id,a2.user_id as user_id,a2.ack_status as ack_status,a1.request_date as request_date,a1.request_project as request_project,a1.created_by as created_by,a1.is_ns as is_ns                                         
							...
							WHERE 1=1 AND a1.is_delete=0 AND a1.rlp_type="Conveyance" AND a2.user_id='.$user_id.' AND a2.is_visible=1 ORDER BY a1.rlp_status DESC,a1.created_at DESC ';
			...
			$sql      = '* FROM `rlp_info` WHERE 1=1 AND `is_delete`=0 AND `rlp_type`="Conveyance" AND created_by='.$user_id.' ORDER BY created_at DESC ';
...
function getRLPListDataW(){
	...
	if($role_name == 'sa'){
			...
			$sql      = 'a1.id as id,a1.rlp_no as rlp_no,a1.rlp_status as rlp_status,a2.rlp_info_id as rlp_info_id,a2.user_id as user_id,a2.ack_status as ack_status,a1.request_date as request_date,a1.request_project as request_project,a1.created_by as created_by,a1.is_ns as is_ns                                         
							FROM `rlp_info` as a1 
							INNER JOIN  `rlp_acknowledgement` as a2 ON a1.id=a2.rlp_info_id
							WHERE 1=1 AND a1.is_delete=0 AND a2.user_id='.$user_id.' AND a2.is_visible=1 ORDER BY a1.rlp_status DESC,a1.created_at DESC ';
			...
			$sql      = '* FROM `rlp_info` WHERE 1=1 AND `is_delete`=0 AND created_by='.$user_id.' ORDER BY created_at DESC ';
...
function getConvRlpDetailsDataForNs($rlp_id){
    $table      =   "rlp_info WHERE id=$rlp_id";
    $rlp_info   = getDataRowIdAndTable($table);
    
    $order = 'asc';
    $column='id';
    $table         =   "rlp_other_cost WHERE rlp_info_id=$rlp_id";
    $rlp_details   = getTableDataByTableNameRlp($table, $order, $column);
