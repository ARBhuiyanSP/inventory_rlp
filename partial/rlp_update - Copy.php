...
?>
										...
										<td><input type="text" name="purpose[]" id="purpose<?php echo $key; ?>" class="form-control" value="<?php echo (isset($editDatas->purpose) && !empty($editDatas->purpose) ? $editDatas->purpose : ''); ?>" <?php if(!is_pro($currentUserId)){ echo 'readonly'; }?> ></td>
										<td><input type="text" name="quantity[]" id="quantity<?php echo $key; ?>" onchange="sum(<?php echo $key; ?>)" class="form-control" value="<?php echo (isset($editDatas->quantity) && !empty($editDatas->quantity) ? $editDatas->quantity : ''); ?>" <?php if(!is_pro($currentUserId)){ echo 'readonly'; }?>></td>
										<td><input type="text" name="unit_price[]" id="unit_price<?php echo $key; ?>" onchange="sum(<?php echo $key; ?>)" class="form-control" value="<?php echo (isset($editDatas->unit_price) && !empty($editDatas->unit_price) ? $editDatas->unit_price : ''); ?>" <?php if(!is_pro($currentUserId)){ echo 'readonly'; }?>></td>
										<td><input type="text" name="amount[]" id="sum<?php echo $key; ?>" class="form-control" value="<?php echo (isset($editDatas->amount) && !empty($editDatas->amount) ? $editDatas->amount : ''); ?>" <?php if(!is_pro($currentUserId)){ echo 'readonly'; }?>></td>
							...
							<textarea class="form-control" rows="3" name="remarks" placeholder="Remarks For Update" required></textarea></br>
							<input type="hidden" name="rlp_info_id" value="<?php echo $_GET['rlp_id'] ?>" />
							<?php } ?>
							<input type="hidden" name="user_id" value="<?php echo $currentUserId; ?>" /> <?php if(is_pro($currentUserId)){ ?>
							<input class="form-control btn btn-info btn-block" type="submit" name="rlp_update_while_approve" value="UPDATE RLP ITEMS" />
							<?php } ?>
                        </div>
                    </div>
					<script>
    ...
    function sum_total() {
        ...
        document.getElementById('allsum').value = newTot.toFixed(2);
            ...
            <div class="col-md-6">
                ...
                <input class="form-control btn btn-info btn-block" type="submit" name="query_submit" value="Ask For More" />
            </div>       
            <?php //} ?>
