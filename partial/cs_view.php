...
<section class="invoice" id="printableArea">
    <div class="row">
        ...
        <div class="col-md-6 col-sm-6">
            ...
            <address>
                ...
                Division:&nbsp;<?php echo getDivisionNameById($cs_info->request_division); ?><br>
                Department:&nbsp;<?php echo getDepartmentNameById($cs_info->request_department) ?><br>
                Project:&nbsp;<?php echo getProjectNameById($cs_info->request_project) ?><br>
        ...
        <div class="col-md-6 col-sm-6">
            <div class="pull-right" style="text-align:right">
                <b>CS NO: &nbsp;<span style=""><?php echo $cs_info->cs_no ?></span></b><br>
                <b>RLP NO: &nbsp;<span style="border:1px solid;padding:2px 5px;"><?php echo $cs_info->rlp_no ?></span></b><br>
                <b>Request Date:</b> <?php echo human_format_date($cs_info->created_at) ?><br><br>
            </div>            
        </div>
        <!-- /.col -->
    </div>
						...
						<tr>
							...
							<th colspan="4" class="text-center" style="background-color:#138496;color:#fff;">
								...
								<button type="button" class="btn btn-xs btn-light" 
										onclick="showAttachment('<?php echo $cs_info->attachment_s1 ?? ''; ?>')">
									📎 Attachment
								</button>
							</th>
							<th colspan="4" class="text-center" style="background-color: #0b621f;color:#fff;">
								Supplier 2 <br>
								<button type="button" class="btn btn-xs btn-light" 
										onclick="showAttachment('<?php echo $cs_info->attachment_s2 ?? ''; ?>')">
									📎 Attachment
								</button>
							</th>
							<th colspan="4" class="text-center" style="background-color: #C82333;color:#fff;">
								Supplier 3 <br>
								<button type="button" class="btn btn-xs btn-light" 
										onclick="showAttachment('<?php echo $cs_info->attachment_s3 ?? ''; ?>')">
									📎 Attachment
								</button>
							</th>
						</tr>
	...
	<div class="row">
					<div class="col-sm-3 col-xs-3" style="padding-top:50px;">
						<center><img src="images/signatures/<?php echo getSignatureByUserId($cs_info->created_by); ?>" height="70px"/></br><?php echo getUserNameByUserId($cs_info->created_by); ?></br>________________________</br><?php echo getDesignationByUserId($cs_info->created_by) ?> [Created By]</center>
					</div>
...
function showAttachment(filePath) {
    ...
    if (['jpg','jpeg','png','gif','webp'].includes(ext)) {
        htmlContent = '<img src="'+filePath+'" style="max-width:100%;height:auto;">';
    } else if (ext === 'pdf') {
        htmlContent = '<embed src="'+filePath+'" type="application/pdf" width="100%" height="600px">';
    } else {
        htmlContent = '<a href="'+filePath+'" target="_blank">Download File</a>';
    }

    // নতুন Window তে খুলবো
    let win = window.open("", "_blank", "width=900,height=700,scrollbars=yes");
