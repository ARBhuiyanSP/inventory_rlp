...
$column = array("ams_products.id", "ams_products.sl_no", "ams_products.item_name", "assets_categories.assets_category", "ams_products.model", "ams_products.assign_status", "store.name");
$query = "
 ...
 INNER JOIN store 
 ON store.id = ams_products.current_store 
";
if($store_id==1){
	$query .= " WHERE 1=1 AND ";
}else{
	$query .= " WHERE 1=1 AND current_store = $store_id AND ";
}
...
if(isset($_POST["search"]["value"]))
{
 ...
 $query .= 'OR store.name LIKE "%'.$_POST["search"]["value"].'%" ';
...
function get_receive_list_action_data($row){
	...
	$edit_url = 'assets_edit.php?edit_id='.$row["id_no"];
